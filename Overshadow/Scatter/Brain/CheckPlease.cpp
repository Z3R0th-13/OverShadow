...
BOOL IsDomainJoined(LPCTSTR domainName)
{
	...
	{
		printf("p\t[-] Machine is not domain joined.\n");
		return FALSE;
	}
	else
	{
...
#define numFiles 32
	...
	printf("=== Checking for VM Drivers ===\n");
...
BOOL HasBadParentProcess()
{
	...
	printf("=== Checking to see if we have a bad parent process ===\n");
	while (pid != 0)
	{
		BOOL found = FALSE;
		HANDLE hSnapshot;
...
#define numSandboxDLLs 6
	...
	printf("=== Checking for Sandbox DLLs ===\n");
	// Get all PIDs
	if (!EnumProcesses(loadedProcesses, sizeof(loadedProcesses), &cbNeeded)) {
		printf("[---] Could not get all PIDs, exiting.\n");
		return FALSE;
...
string MotherFuckingBreadCrumbs = "I love bread";
...
BOOL SafeToExecute()
{
	...
	//BOOL domCheck = IsDomainJoined(); if (!domCheck) { printf("\t[-] Computer is not domain joined.\n"); ReturnFlag = FALSE; }
	//if (HasBadParentProcess()) { printf("\t[-] [SANDBOX/DEBUG] The current process has non-Microsoft parent processes.\n"); ReturnFlag = FALSE; } else { printf("\t[+] Parent process tree is clean and valid!\n"); } 
	return ReturnFlag;
}