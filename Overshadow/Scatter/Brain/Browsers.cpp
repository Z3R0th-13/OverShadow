...
static wchar_t* GetGoogleChromePath()
{
	char* path;
...
static wchar_t* GetGoogleChromeSxSPath()
{
	DWORD sz = sizeof(char) * MAX_PATH;
	char* path;
	char* chromeExePath = (char*)malloc(sz);
...
static wchar_t* GetIEPath()
{
	DWORD sz = sizeof(char) * MAX_PATH;
	char* iexploreExePath = (char*)malloc(sz);
	char* path;
	size_t len;
	char ffPath[] = "\\Internet Explorer\\iexplore.exe";
	_dupenv_s(&path, &len, "PROGRAMFILES");
	strcpy_s(iexploreExePath, sz, path);
	free(path);
	strcat_s(iexploreExePath, sz, ffPath);

	wchar_t* result = CharToWcharT(iexploreExePath);
	free(iexploreExePath);
	if (FileExistsAndIs64Bit(result))
	{
		return result;
	}
...
static wchar_t* GetOperaPath()
{
	DWORD sz = sizeof(char) * MAX_PATH;
	char* operaExePath = (char*)malloc(sz);
	char* path;
	size_t len;
	char ffPath[] = "\\Opera\\Launcher.exe";
	_dupenv_s(&path, &len, "PROGRAMFILES");
	strcpy_s(operaExePath, sz, path);
	free(path);
	strcat_s(operaExePath, sz, ffPath);

	wchar_t* result = CharToWcharT(operaExePath);
	free(operaExePath);
	if (FileExistsAndIs64Bit(result))
	{
		return result;
...
static wchar_t* GetDefaultBrowser()
{
